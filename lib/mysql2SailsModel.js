...
module.exports = {
  "convertSchema2SailsModel": function(schema){
    ...
    for(var i=0;i<schemaLength;i++){
      ...
      } else if( _schema.Key === "UNI"){
        attributes[camelizedField].unique = true;
      }

      if(fieldDefaultValue) {
        attributes[camelizedField].defaultsTo = fieldDefaultValue;

        if (attributes[camelizedField].type == 'integer') {
            attributes[camelizedField].defaultsTo = parseInt(fieldDefaultValue, 0);
        }
      }
